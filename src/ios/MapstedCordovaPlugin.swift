...
// At SDK 26.7.1 the RoutingRequestCallback protocol takes MNRouteResponse on both
// onSuccess/onError, and MapstedMapApi conforms to the callback itself (the old
// handleRouteResponse/handleRouteError entry points were removed), so these are empty.
